...
- **ページ遷移を跨いで維持**: 同一タブ内で別ページへ遷移 (SPA 遷移・リロード・通常のリンク遷移) しても ON のまま維持される。キャプチャが失われて再取得もできない場合のみ自動で OFF になり、ツールバーバッジ「OFF」で通知する
...
- 別オリジンのページへ遷移した後にキャプチャが失われた場合は、activeTab の付与が切れているため自動復旧できず OFF に戻る (popup から再度 ON にする)
